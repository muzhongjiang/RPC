...
[![Actions Status](https://github.com/xuxueli/rpc/workflows/Java%20CI/badge.svg)](https://github.com/xuxueli/rpc/actions)
[![Maven Central](https://maven-badges.herokuapp.com/maven-central/com.mzj/rpc/badge.svg)](https://maven-badges.herokuapp.com/maven-central/com.mzj/rpc/)
[![GitHub release](https://img.shields.io/github/release/xuxueli/rpc.svg)](https://github.com/xuxueli/rpc/releases)
[![GitHub stars](https://img.shields.io/github/stars/xuxueli/rpc)](https://github.com/xuxueli/rpc/)
[![Docker Status](https://img.shields.io/docker/pulls/xuxueli/rpc-admin)](https://hub.docker.com/r/xuxueli/rpc-admin/)
...
- [中文文档](https://www.xuxueli.com/rpc/)

#### 源码仓库地址

源码仓库地址 | Release Download
--- | ---
[https://github.com/xuxueli/rpc](https://github.com/xuxueli/rpc) | [Download](https://github.com/xuxueli/rpc/releases)
[https://gitee.com/xuxueli0323/rpc](https://gitee.com/xuxueli0323/rpc) | [Download](https://gitee.com/xuxueli0323/rpc/releases)  
...
解压源码,按照maven格式将源码导入IDE, 使用maven进行编译即可，源码结构如下：
    ...
    源码目录介绍：
    - /doc
    - /rpc-admin    ：轻量级服务（注册）中心，可选模块；
    - /rpc-core     ：核心依赖；
    - /rpc-samples  ：示例项目；
        - /rpc-sample-frameless     ：无框架版本示例；
        - /rpc-sample-springboot    ：springboot版本示例；
            - /rpc-sample-springboot-api           ：公共API接口
            - /rpc-sample-springboot-client        ：服务消费方 invoker 调用示例；
            - /rpc-sample-springboot-server        ：服务提供方 provider 示例;

### 2.2 搭建部署 "服务(注册)中心"      
推荐使用 "rpc-admin" 作为轻量级服务(注册)中心。非常轻量级，一分钟可完成部署工作。
注册中心为可选模块，可以不使用注册中心，也可以选型其他注册中心。

#### 2.2.1 初始化“服务(注册)中心数据库”
请下载项目源码并解压，获取 "调度数据库初始化SQL脚本" 并执行即可。数据库初始化SQL脚本"位置为:

    /rpc/doc/db/tables_xxl_rpc.sql

服务(注册)中心支持集群部署，集群情况下各节点务必连接同一个mysql实例;如果mysql做主从,集群节点务必强制走主库;

#### 2.2.2 配置部署“服务(注册)中心”

    服务(注册)中心项目：rpc-admin
...
```
/rpc/rpc-admin/src/main/resources/application.properties
...
### 服务注册数据磁盘同步目录
xxl.rpc.registry.data.filepath=/data/applogs/rpc/registrydata
### rpc, access token
...
访问地址：http://localhost:8080/rpc-admin  (该地址接入方项目将会使用到，作为注册地址)，登录后运行界面如下图所示

![输入图片说明](https://www.xuxueli.com/doc/static/rpc/images/img_01.png "在这里输入图片标题")
...
```
// Docker地址：https://hub.docker.com/r/xuxueli/rpc-admin/
docker pull xuxueli/rpc-admin
```

- 创建容器并运行

```
docker run -p 8080:8080 -v /tmp:/data/applogs --name rpc-admin  -d xuxueli/rpc-admin

/**
* 如需自定义 mysql 等配置，可通过 "PARAMS" 指定，参数格式 RAMS="--key=value  --key2=value2" ；
* 配置项参考文件：/rpc/rpc-admin/src/main/resources/application.properties
*/
docker run -e PARAMS="--spring.datasource.url=jdbc:mysql://127.0.0.1:3306/xxl_rpc?Unicode=true&characterEncoding=UTF-8" -p 8080:8080 -v /tmp:/data/applogs --name rpc-admin  -d xuxueli/rpc-admin
```
 
### 2.3 项目中使用XXL-RPC
以示例项目 “rpc-sample-springboot” 为例讲解；
...
```
// 参考代码位置：com.xxl.rpc.sample.server.conf.RpcProviderConfig
...
实现 “服务API” 的接口，开发业务逻辑代码；
    ...
    2、添加 “@RpcService” 注解：被 “XXL-RPC” 的 ProviderFactory 扫描识别，进行Provider服务注册，如果开启注册中心同时也会进行注册中心服务注册； 
...
```
// 参考代码位置：com.xxl.rpc.sample.client.conf.RpcInvokerConfig
...
以示例项目 “rpc-sample-frameless” 为例讲解；该示例项目以直连IP方式进行演示，也可以选择接入注册中心方式使用。

### 3.1 API方式创建“服务提供者”：
```
// 参考代码位置：com.xxl.rpc.sample.server.RpcServerApplication
...
```
// 参考代码位置：com.xxl.rpc.sample.client.RpcClientAplication
...
### 4.1 系统架构图
![输入图片说明](https://www.xuxueli.com/doc/static/rpc/images/img_DNq6.png "在这里输入图片标题")
...
### 4.4 RPC工作原理剖析
![输入图片说明](https://www.xuxueli.com/doc/static/rpc/images/img_XEVY.png "在这里输入图片标题")
...
### 4.5 TCP通讯模型
![输入图片说明](https://www.xuxueli.com/doc/static/rpc/images/img_b1IX.png "在这里输入图片标题")
...
### 4.6 sync-over-async
![输入图片说明](https://www.xuxueli.com/doc/static/rpc/images/img_pMtS.png "在这里输入图片标题")
...
![输入图片说明](https://www.xuxueli.com/doc/static/rpc/images/img_m3Ma.png "在这里输入图片标题")
...
- b、修改通讯枚举，需要同时在“服务方”与“消费方”两端一同修改，通讯枚举属性代码位置如下：
    - 服务工厂 "RpcSpringProviderFactory.netType" ：可参考springboot示例组件初始化代码；
    - 服务引用注解 "RpcReference.netType" | 服务Bean对象 "RpcReferenceBean.netType" ：可参考springboot示例组件初始化代码；
...
- b、修改注册中心配置，需要同时在“服务方”与“消费方”两端一同修改，代码位置如下：
    - RpcSpringProviderFactory.serviceRegistryClass：注册中心实现类，可选：XxlRegistryServiceRegistry.class、LocalServiceRegistry.class、ZkServiceRegistry.class
    - RpcSpringProviderFactory.serviceRegistryParam：注册中心启动参数，各种注册中心启动参数不同，可参考其 start 方案了解；
    

### 4.11 泛化调用
XXL-RPC 提供 "泛化调用" 支持，服务调用方不依赖服务方提供的API；泛化调用通常用于框架集成，比如 "网关平台、跨语言调用、测试平台" 等；
开启 "泛化调用" 时服务方不需要做任何调整，仅需要调用方初始化一个泛化调用服务Reference （"RpcGenericService"） 即可。


“RpcGenericService#invoke” 请求参数 | 说明
--- | ---
String iface | 服务接口类名
String version | 服务版本
String method | 服务方法
...
RpcGenericService genericService = (RpcGenericService) new RpcReferenceBean(……).getObject();
...
### 5.6 服务中心系统设计
#### 5.6.1 系统架构图
![输入图片说明](https://www.xuxueli.com/doc/static/rpc/images/img_02.png "在这里输入图片标题")
...
- 2、除了springboot类型示例；新增无框架示例项目 "rpc-sample-frameless"。不依赖第三方框架，只需main方法即可启动运行；
...
- 3、POM依赖升级，冗余POM清理；
- 4、代码优化：RpcInvokerFactory 移除 static 代码块及相关组件，进一步实现组件无状态；
- 5、服务注册逻辑优化，避免地址重复生成；
...
- 3、新增jfinal类型示例项目 "rpc-sample-jfinal" 支持jfinal项目快速接入分布式RPC服务功能；高兼容性，原则上支持任务框架，甚至main方法直接运行；
...
- 1、为方便维护，合并xxl-registry至rpc，模块名为rpc-admin;
...
- rpc时钟参数仅记录，取消时钟校验逻辑；
- 调用链追踪，监控；结合 xxl-apm 与 rpc filter共同演进；
- 限流-熔断-降级，结合xxl-registry与rpc filter共同演进；
...
### 7.1 项目贡献
欢迎参与项目贡献！比如提交PR修复一个bug，或者新建 [Issue](https://github.com/xuxueli/rpc/issues/) 讨论新特性或者变更。

### 7.2 用户接入登记
更多接入的公司，欢迎在 [登记地址](https://github.com/xuxueli/rpc/issues/2 ) 登记，登记仅仅为了产品推广。
